// Idempotent bootstrap for the Functional 05 throwaway test merchant.
// Creates (or reuses) soho-crew-test+f5@soho.test and ensures it owns a dedicated
// test merchant. NEVER touches the two real seeded merchants.
// Run: node --env-file=.env test/f5-setup.mjs
import { createClient } from "@supabase/supabase-js";
import pg from "pg";

const URL = process.env.VITE_SUPABASE_URL;
const KEY = process.env.VITE_SUPABASE_ANON_KEY;
export const F5_EMAIL = "soho-crew-test+f5@soho.test";
export const F5_PASSWORD = "SohoF5Test!2026";
export const REAL_MERCHANTS = ["4e63a397-e811-48b1-86e5-d7fc5ffa9f0e", "830effcb-2d6b-4e99-9b0f-07471755e60d"];

export async function ensureF5Merchant() {
  const sb = createClient(URL, KEY, { auth: { persistSession: false } });
  let si = await sb.auth.signInWithPassword({ email: F5_EMAIL, password: F5_PASSWORD });
  if (si.error) {
    const su = await sb.auth.signUp({ email: F5_EMAIL, password: F5_PASSWORD });
    if (su.error) throw new Error("signup: " + su.error.message);
    si = await sb.auth.signInWithPassword({ email: F5_EMAIL, password: F5_PASSWORD });
  ...
  try {
    ...
    if (merchantId && REAL_MERCHANTS.includes(merchantId)) {
      throw new Error("REFUSING: f5 test user resolved to a REAL merchant id");
    }
    if (!merchantId) {
      const ins = await pool.query(
        ...
        ["Cửa hàng Test F5", "Test F5", userId],
      );
      ...
      // A minimal merchant_settings row so F3 sale flows have a context.
      await pool.query(
        `insert into public.merchant_settings (merchant_id, timezone, receipt_prefix, allow_cash, allow_qr)
         values ($1,'Asia/Ho_Chi_Minh','F5T', true, true) on conflict (merchant_id) do nothing`,
        [merchantId],
      );
    }
    if (REAL_MERCHANTS.includes(merchantId)) throw new Error("REFUSING: resolved a REAL merchant id");
    return { userId, merchantId, token: session.access_token, email: F5_EMAIL };
  } finally {
    await pool.end();
  }
}

// Idempotent bootstrap for the Functional 12 throwaway test merchants.
// Creates (or reuses) soho-crew-test+f12@soho.test (primary) and +f12b (for the
// RLS cross-tenant test), each owning a dedicated test merchant. NEVER touches the
// two real seeded merchants. Run: node --env-file=.env test/f12-setup.mjs
...
async function ensure(email, password, displayName) {
  ...
  try {
    ...
    if (merchantId && REAL_MERCHANTS.includes(merchantId)) throw new Error("REFUSING: f12 test user resolved to a REAL merchant id");
    if (!merchantId) {
    ...
    return { userId, merchantId, token: session.access_token, email };
  } finally {
    await pool.end();
  }
}

export const F12_EMAIL = "soho-crew-test+f12@soho.test";
export const F12B_EMAIL = "soho-crew-test+f12b@soho.test";

export function ensureF12Merchant() {
  return ensure(F12_EMAIL, "SohoF12Test!2026", "Test F12");
}
export function ensureF12bMerchant() {
  return ensure(F12B_EMAIL, "SohoF12bTest!2026", "Test F12b");
}

if (import.meta.url === `file://${process.argv[1]}`) {
  Promise.all([ensureF12Merchant(), ensureF12bMerchant()])
    .then(([a, b]) => { console.log(JSON.stringify({ a, b }, null, 2)); process.exit(0); })
